...
% To avoid the usage of the Multichannel Impulse Response Database,
% comment lines 38--44 and uncomment line 35 (the responses saved in
% impresponses.mat will be used)

% load impresponses

% Speaker
angle = 0;
a1 = SP.loadImpulseResponse(pathData, micSpac, angle, dist, T60, 0.6);
a1.resample_it(Fs);a1 = a1.to_double();a1 = a1(:,micsUsed);
% Noise
angle = 315;
a2 = SP.loadImpulseResponse(pathData, micSpac, angle, dist, T60, 0.6);
a2.resample_it(Fs);a2 = a2.to_double();a2 = a2(:,micsUsed);


    
% Speaker Spatial Images    
s1 = audioread(soundPath);
...
for lp = 1:2, resp(lp,:,1) = filter(a1(:,lp),1,s1)'; end

% Noise Spatial Images
s1 = randn(size(s1)); for lp=1:2, resp(lp,:,2) = filter(a2(:,lp),1,s1)';end
% Setting initial SNR
